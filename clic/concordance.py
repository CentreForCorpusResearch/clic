...
class Concordance(object):
    ...
    def create_concordance(self, terms, idxName, Materials, selectWords, page_number, page_size):
        """
        ...
        if len(result_set) > 0:
            count = 0
            start_result = ((page_number - 1) * page_size) + 1
            end_result = (start_result - 1) + page_size
    ...
    def create_concordance_ajax(self, terms, idxName, Materials, selectWords, page_number, page_size):
        """
        main concordance method
        create a list of lists containing each three contexts left - node -right,
        and a list within those contexts containing each word.
            ...
            for result in result_set:
                ...
                for match in result.proxInfo:
                    ...
                    conc_line = {
                        ...
                        'para_chap': para_chap,
                        'sent_chap': sent_chap,
                        'word_chap': str(word_chap),
                        'chap_word_count': str(chapWordCount),
                        'para_book': str(para_book),
                        'sent_book': str(sent_book),
                        'total_word': str(total_word)
                        }
